...
class CustomDeviseMailer < Devise::Mailer
  ...
  def confirmation_instructions(record, token, _opts = {})
    sendgrid_devise_mail(:confirmation_instructions, record, user_confirmation_url(user: record, confirmation_token: token))
  end

  def reset_password_instructions(record, token, _opts = {})
    sendgrid_devise_mail(:reset_password_instructions, record, edit_user_password_url(user: record, reset_password_token: token))
  end
